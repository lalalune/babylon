...
test.describe('OG Embed Sharing', () => {
  test.describe('P&L Sharing', () => {
    ...
    test('should have Farcaster Frame meta tags on P&L share page', async ({ page }) => {
      const testUserId = 'test-user-id'
      await page.goto(`${BASE_URL}/share/pnl/${testUserId}`, {
        waitUntil: 'domcontentloaded',
      })
      
      // Check for Farcaster Frame tags
      const fcFrame = await page.locator('meta[property="fc:frame"]').getAttribute('content')
      const fcImage = await page.locator('meta[property="fc:frame:image"]').getAttribute('content')
      const fcButton = await page.locator('meta[property="fc:frame:button:1"]').getAttribute('content')
      
      expect(fcFrame).toBe('vNext')
      expect(fcImage).toContain('/api/og/pnl/')
      expect(fcButton).toBe('View on Babylon')
    })
  ...
  test.describe('Referral Sharing', () => {
    ...
    test('should have Farcaster Frame meta tags on referral share page', async ({ page }) => {
      const testUserId = 'test-user-id'
      await page.goto(`${BASE_URL}/share/referral/${testUserId}`, {
        waitUntil: 'domcontentloaded',
      })
      ...
      expect(fcFrame).toBe('vNext')
      expect(fcImage).toContain('/api/og/referral/')
      expect(fcButton).toBe('Join Babylon')
    })
  ...
  test.describe('Share Link Redirects', () => {
    ...
    test('should redirect referral share page with ref code', async ({ page }) => {
      const testUserId = 'test-user-id'
      
      await page.goto(`${BASE_URL}/share/referral/${testUserId}`)
      
      // Should redirect to home with ref parameter
      await page.waitForURL(/\/\?ref=/, { timeout: 5000 })
