...
API Route: /api/upload/image Methods: POST (upload image) Handles image uploads for user profiles (avatar, cover images) Uses S3-compatible storage (MinIO for dev, Cloudflare R2 for production) ⚠️  Vercel-compatible: No local filesystem fallback on production
