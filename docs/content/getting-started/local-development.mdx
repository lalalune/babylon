...
Runs both the web server and the game engine daemon:
...
- Start the game engine daemon (generates content every 60 seconds)
...
Test the Vercel cron behavior locally:

```bash
bun run dev:cron-mode
```

This simulates how the app runs in production on Vercel with cron jobs instead of a daemon.
